        ...
        try:
                ...
                if data == "reset":
                    ...
                    if idx == game.player.equipped_weapon_index:
                        add_log(game, "Нельзя продать надетое оружие.")
                    elif 0 <= idx < len(game.player.weapon_inventory):
                    ...
                    idx = int(data.split(":", 1)[1])
                    if idx == game.player.equipped_weapon_index:
                        add_log(game, "Нельзя разобрать надетое оружие.")
                    elif 0 <= idx < len(game.player.weapon_inventory):
                        gear = game.player.weapon_inventory[idx]
                        reward = salvage_reward(gear)
                        del game.player.weapon_inventory[idx]
                        if game.player.equipped_weapon_index is not None and game.player.equipped_weapon_index > idx:
                    ...
                    equipped_armor_indexes = {
                        equipped_idx
                        for equipped_idx in game.player.equipped_armor.values()
                        if equipped_idx is not None
                    }

                    if idx in equipped_armor_indexes:
                        add_log(game, "Нельзя разобрать надетую броню.")
                    elif 0 <= idx < len(game.player.armor_inventory):
                        ...
                        for slot, equipped_idx in list(game.player.equipped_armor.items()):
                            if equipped_idx is not None and equipped_idx > idx:
